...
公式のデプロイ経路はコンテナ化です。詳細は [docs/deployment/containerized.md](docs/deployment/containerized.md) を参照してください。内容は次をまとめています。

- `docker compose up -d --build`
- `scripts/deploy/start_container_stack.sh`
- `scripts/deploy/start_container_stack.ps1`
- `.env.example` から `.env` への準備手順

ネイティブ開発は引き続き利用できますが、デプロイとは分離し、`scripts/startup/` とフロントエンド開発サーバーを使います。
