...
**Goal:** Rewrite README.md and README.zh-CN.md with corrected script paths, removed stale references, and Usage limited to Web only.

**Architecture:** Direct file rewrites. No code changes. Both files updated in parallel tasks, committed together.
...
- Modify: `README.zh-CN.md` — full rewrite (Chinese mirror)
...
```bash
git add README.md
git commit -m "docs(readme): rewrite for clarity — fix script paths, web-only usage"
```
...
- [ ] **Step 1: Write the new README.zh-CN.md**

Replace the entire file with the Chinese mirror of Task 1:
...
git commit -m "docs(readme): rewrite zh-CN mirror — fix script paths, web-only usage"
```
