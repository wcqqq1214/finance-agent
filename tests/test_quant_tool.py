...
def test_run_ml_quant_analysis_includes_historical_similarity(monkeypatch):
    ...
    assert result["signal_filter"]["ml_policy"] == "primary_signal"
...
def test_run_ml_quant_analysis_degrades_to_event_driven_only_for_weak_auc(monkeypatch):
    panel_df = pd.DataFrame(
        {
            "symbol": pd.Series(["NVDA", "MSFT", "NVDA", "MSFT"], dtype="category"),
            "trade_date": pd.to_datetime(["2024-01-02", "2024-01-02", "2024-01-03", "2024-01-03"]),
            "close": [100.0, 200.0, 103.0, 198.0],
            "ret_1d": [0.1, 0.0, 0.2, -0.1],
            "ret_3d": [0.1, 0.0, 0.2, -0.1],
            ...
            "news_count_3d_residual": [0.5, -0.5, 0.5, -0.5],
            "ret_1d_residual": [0.05, -0.05, 0.15, -0.15],
            "news_text_blob": ["chip demand", "macro slowdown", "ai demand", "cloud pressure"],
            "target_up_big_move_t3": [1, 0, 1, 0],
        }
    )
    ...
    result = quant_tool._run_ml_quant_analysis_impl("nvda")

    assert result["final_prob_up"] == 0.5
    assert result["final_prediction"] == "event_driven_only"
    assert result["ml_policy"] == "event_driven_only"
    assert result["signal_filter"]["position_multiplier"] == 0.0
    assert result["signal_filter"]["ml_signal_enabled"] is False
    assert "EVENT_DRIVEN_ONLY" in result["markdown_report"]
